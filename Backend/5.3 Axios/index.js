...
app.post("/", async (req, res) => {
  ...
  if(req.body['type'] === '' && req.body['participants'] === ''){  
    console.log("Random event has been selected");
    res.redirect("/");
  }
  else{
    if(req.body['type'] !== ''){
      console.log("Filter has been used");
      apiURL += '?type=' + req.body['type'];
      if(req.body['participants'] !== ''){
        apiURL += '&participants=' + req.body['participants'];
      }
    }
    else{
      apiURL += '?&participants=' + req.body['participants'];
    ...
    try {
      ...
      console.error("Failed to make request:", error.message);
      res.render("index.ejs", {
        error: 'No match can be found of your criteria',
      });
    }
  }
});
